...
export const StructuredCV: React.FC<StructuredCVProps> = ({ data, theme }) => {
    // Basic guards for top-level objects
    // Basic guards for top-level objects with safe defaults
    const personal = data?.personal || ({} as any);
    const narrative = data?.narrative || ({} as any);
    ...
    const isCompact = narrative?.style_meta?.length === 'short';

    return (
        <div className={`${isCompact ? 'space-y-4' : 'space-y-8'} ${theme.container}`}>
            {/* Header */}
            <header className={`${theme.header} space-y-4`}>
                <div className="flex justify-between items-start gap-6">
                    ...
                    <div className={`${isCompact ? 'space-y-1' : 'space-y-2'} flex-1`}>
                        <h1 className={`${isCompact ? 'text-2xl' : 'text-4xl'} font-bold tracking-tight text-slate-900`}>{personal?.name}</h1>
                        <p className={`${isCompact ? 'text-lg' : 'text-xl'} font-medium ${theme.accent}`}>{personal?.title}</p>
            ...
            {/* Narrative Section */}
            {(narrative?.short || narrative?.long) && (
                <section className="space-y-3">
                    <h2 className={theme.sectionTitle}>Professional Narrative</h2>
                    <div className={`space-y-3 ${theme.text}`}>
                        {narrative?.short && <p className="font-medium italic border-l-2 border-slate-200 pl-4">{narrative.short}</p>}
                        {narrative?.long && narrative?.style_meta?.length === 'long' && (
            ...
            <div className={`grid grid-cols-3 ${isCompact ? 'gap-4' : 'gap-8'}`}>
                {/* Main Column */}
                <div className={`col-span-2 ${isCompact ? 'space-y-4' : 'space-y-8'}`}>
                    {/* Education (Moved up for emphasis) */}
                    ...
                    {experience && experience.length > 0 && (
                        <section className={isCompact ? 'space-y-3' : 'space-y-6'}>
                            <h2 className={theme.sectionTitle}>Experience</h2>
                            <div className={isCompact ? 'space-y-4' : 'space-y-8'}>
                                {experience?.map((exp, idx) => (
                                    <div key={idx} className={`${theme.timeline} relative break-inside-avoid`}>
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="text-right">
                                                    <span className={`${theme.badge} ${isCompact ? 'text-[9px] px-1 py-0' : ''}`}>{exp?.start_date} – {exp?.end_date}</span>
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {projects?.map((proj, idx) => (
                                    <div key={idx} className={`${theme.card} break-inside-avoid`}>
                        ...
                        <section className="space-y-3">
                            ...
                            <div className="space-y-2">
                                {languages?.map((lang, idx) => (
                                    <div key={idx} className="flex justify-between items-center text-xs break-inside-avoid">
